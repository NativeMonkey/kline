...
public class CandleDrawing implements IDrawing {
    ...
    private float candleSpace = 0f; // entry 与 entry 之间的间隙，默认 0.1f (10%)
...
//        }
        ...
        for (int i = minIndex; i < maxIndex; i++) {
            ...
            if (Math.abs(candleRectBuffer[1] - candleRectBuffer[3]) < 1.f) { // 涨停、跌停、或不涨不跌的一字板
                canvas.drawRect(candleRectBuffer[0]+2f, candleRectBuffer[1], candleRectBuffer[2]-2f, candleRectBuffer[3] + 2, candlePaint);
            } else {
                canvas.drawRect(candleRectBuffer[0]+2f, candleRectBuffer[1], candleRectBuffer[2]-2f, candleRectBuffer[3], candlePaint);
            }
            ...
            if (render.isHighlight()) {
                ...
                if (candleRectBuffer[0] <= highlightPoint[0] && highlightPoint[0] <= candleRectBuffer[2]) {
                    ...
                    if (entry.getOpen() > entry.getClose()) {
                        highlightPoint[1] =  candleRectBuffer[3];
                    } else {
                        highlightPoint[1] =  candleRectBuffer[1];
                    }
